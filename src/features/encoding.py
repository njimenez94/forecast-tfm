...
if __name__ == "__main__":
    ...
    _dates = [_dt.date(2020, 1, 1) + _dt.timedelta(days=i) for i in range(5)]
    _df = pl.DataFrame({
        "series_id": ["S1"] * 5 + ["S2"] * 5,
        "item_id": ["I1"] * 5 + ["I2"] * 5,
        "dept_id": ["D1"] * 10,
    ...
    _out = add_encoding_features(_df)

    # día 1: sin historia previa del item -> null (no leakea el propio día)
    assert _out.filter(pl.col("date") == _dates[0])["te_item_mean"].is_null().all()
    ...
    i1 = _out.filter(pl.col("item_id") == "I1").sort("date")
    ...
    i2 = _out.filter(pl.col("item_id") == "I2").sort("date")
    ...
    s1 = _out.filter(pl.col("series_id") == "S1").sort("date")
