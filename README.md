...
Flujo: descomprimir datos → crear DB → construir datasets por nivel → generar features derivadas → entrenar/evaluar por nivel → generar test set final.

## Uso típico

```bash
make create-database                          # 1. descomprime backup/*.zip -> data/m5.db
...
|---|--------|--------|----------|--------|
| 1 | [scripts/create_database.py](scripts/create_database.py) | `create-database` | Descomprime `backup/m5-forecasting-accuracy.zip` en `data/raw/` y crea la base DuckDB. | `data/m5.db` |
