...
<!-- ⚠️ REVISAR ANTES DE ENTREGAR: esta tabla (y las de la sección 7/8) se generaron antes de corregir 3 fugas de datos encontradas en revisión de código: `is_store_closed` usaba la venta de la propia fila y una media de toda la serie (train+valid+test) para "confirmar" un cierre de tienda que además ponía a 0 el target (src/features/events.py); `price_vs_mean` promediaba el precio sobre toda la serie sin acotar por fecha, incluyendo test (src/data/base_query.py); `days_since_release` podía dar "días hasta el release" (una fecha futura) en filas previas al lanzamiento del producto (src/features/events.py). Las tres ya están corregidas y los datasets/artifacts regenerados -- reemplazar estos números por los de la corrida post-fix antes de la entrega. -->
...
<!-- ⚠️ REVISAR ANTES DE ENTREGAR: la selección de familia de modelo, la eliminación backward de variables y la búsqueda de Optuna se evalúan todas, repetidamente, contra la misma ventana fija de validación (2015-04-27 a 2016-04-24) -- no hay CV anidada en estas etapas (el rolling-CV de src/data/temporal_split.py::rolling_cv_folds solo se usa para elegir el nº de árboles final). Con cientos de evaluaciones sobre una única ventana existe riesgo de "researcher degrees of freedom" (sobreajuste de la selección de variables/hiperparámetros a esa ventana particular, no a la relación subyacente). El chequeo de test al final mitiga pero no elimina el riesgo, porque no participa en ninguna decisión previa. Vale la pena nombrar esta limitación explícitamente en la defensa en vez de solo mencionar la diferencia de ventana entre test y validación (ver nota al pie de la tabla de test en la sección 7 y el bullet correspondiente en la sección 10). -->
...
- **Ventana única de validación.** La selección de familia de modelo, la eliminación *backward* de variables y la optimización de hiperparámetros con Optuna se evalúan todas contra la misma ventana fija de validación, sin validación cruzada anidada en esas etapas (el *rolling*-CV ya implementado en el pipeline solo se usa para fijar el número final de árboles). Esto introduce cierto riesgo de que la variable/hiperparámetro elegido esté ligeramente sobreajustado a esa ventana particular en vez de a la relación subyacente. Extender el *rolling*-CV a las etapas de selección de familia/variables es la línea de trabajo más directa para cerrar esta brecha.
